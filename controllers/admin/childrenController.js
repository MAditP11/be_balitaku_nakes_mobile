  ...
  await models.Children.findAll({
    ...
    normal.forEach((element) => {
      ...
      if (element.status === a.status) {
        ...
        console.log('maaf tidak ada status anak yang cocok dengan data status yang ada di check up');
      }
    });
  ...
  models.Children.findByPk(req.params.id, {
    include: [
      'user',
      {
        model: models.Checkup,
        as: 'checkup',
        include: ['posyandu', { model: models.Kader, as: 'kader', include: ['user'] }, { model: models.Vaccination, as: 'vaccination', include: ['vaccine'] }, { model: models.AksiPenanganan, as: 'aksi_penanganan' }],
      },
    ],
  }).then((result) => {
    res.status(200).json({
